#===============================================================================
# R脚本: BLAST结果解析与L-R同源映射
# 功能: 将拟南芥L-R对映射到毛果杨
# 使用方法: Rscript 02_LR_mapping.R
#===============================================================================
...
blast_clean <- blast_raw %>%
  mutate(
    # 拟南芥: 提取基因ID (去掉转录本版本号和PACid)
    ath_gene = sub("\\|.*", "", qseqid),           # 去掉|后面的内容
    ath_gene = sub("\\.\\d+$", "", ath_gene),      # 去掉.1等版本号
    
    # 杨树: 提取基因locus
    ptr_gene = sub("\\.p$", "", sseqid),           # 去掉.p后缀
    ptr_gene = sub("\\.\\d+$", "", ptr_gene),      # 去掉转录本版本号
    
    # 保留原始转录本ID用于追溯
    ath_transcript = sub("\\|.*", "", qseqid),
    ptr_transcript = sub("\\.p$", "", sseqid)
  )

cat(sprintf("  唯一拟南芥基因: %d\n", n_distinct(blast_clean$ath_gene)))
cat(sprintf("  唯一杨树基因: %d\n", n_distinct(blast_clean$ptr_gene)))
...
# 方法1: Reciprocal Best Hit (RBH) - 更严格
# 方法2: Best Hit by bitscore - 本脚本采用

best_hits <- blast_filtered %>%
  group_by(ath_gene) %>%
  arrange(desc(bitscore), evalue) %>%
  slice_head(n = 1) %>%       # 每个拟南芥基因只保留最佳匹配
  ungroup()
...
if (ncol(lr_ath) >= 2) {
  colnames(lr_ath)[1:2] <- c("Ligand_Ath", "Receptor_Ath")
}

# 清洗L-R基因ID (与BLAST结果格式统一)
lr_ath <- lr_ath %>%
  mutate(
    Ligand_Ath_clean = sub("\\.\\d+$", "", Ligand_Ath),
    Receptor_Ath_clean = sub("\\.\\d+$", "", Receptor_Ath)
  )

cat(sprintf("  L-R对数: %d\n", nrow(lr_ath)))
cat(sprintf("  唯一Ligand: %d\n", n_distinct(lr_ath$Ligand_Ath_clean)))
cat(sprintf("  唯一Receptor: %d\n", n_distinct(lr_ath$Receptor_Ath_clean)))
...
lr_ptr <- lr_ath %>%
  mutate(
    Ligand_Ptr = ath2ptr[Ligand_Ath_clean],
    Receptor_Ptr = ath2ptr[Receptor_Ath_clean],
    
    # 标记映射状态
    Ligand_Mapped = !is.na(Ligand_Ptr),
    Receptor_Mapped = !is.na(Receptor_Ptr),
    Both_Mapped = Ligand_Mapped & Receptor_Mapped
  )
...
lr_ptr_full <- lr_ptr %>%
  left_join(
    best_hits %>% select(ath_gene, pident, qcovs, evalue, bitscore),
    by = c("Ligand_Ath_clean" = "ath_gene")
  ) %>%
  rename(
  ...
  left_join(
    best_hits %>% select(ath_gene, pident, qcovs, evalue, bitscore),
    by = c("Receptor_Ath_clean" = "ath_gene")
  ) %>%
  rename(
...
# 9.4 未映射的基因列表 (便于排查)
unmapped_ligands <- lr_ath %>%
  filter(!Ligand_Ath_clean %in% names(ath2ptr)) %>%
  pull(Ligand_Ath_clean) %>%
  unique()

unmapped_receptors <- lr_ath %>%
  filter(!Receptor_Ath_clean %in% names(ath2ptr)) %>%
  pull(Receptor_Ath_clean) %>%
  unique()
